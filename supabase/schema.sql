...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'friendships_not_self_check'
  ) then
    alter table public.friendships
      add constraint friendships_not_self_check check (requester_id <> addressee_id);
  end if;
end;
$$;
...
drop policy if exists "check_ins_select_public_friend_or_own" on public.check_ins;
...
create policy "check_ins_select_public_friend_or_own" on public.check_ins for select using (
  status = 'public'
  or auth.uid() = user_id
  or exists (
    select 1 from public.follows mine
    join public.follows theirs
      on theirs.follower_id = check_ins.user_id and theirs.following_id = auth.uid()
    where mine.follower_id = auth.uid() and mine.following_id = check_ins.user_id
  )
);
...
drop policy if exists "events_select_public_friend_or_owner" on public.events;
drop policy if exists "events_insert_auth" on public.events;
drop policy if exists "events_update_owner" on public.events;
drop policy if exists "events_delete_owner" on public.events;
create policy "events_select_public_friend_or_owner" on public.events for select using (
  visibility = 'public'
  or auth.uid() = created_by
  or exists (
    select 1 from public.follows mine
    join public.follows theirs
      on theirs.follower_id = events.created_by and theirs.following_id = auth.uid()
    where mine.follower_id = auth.uid() and mine.following_id = events.created_by
  )
);
