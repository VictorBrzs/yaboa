...
3. Execute `npm run criar:convidado`. Ele cria ou corrige apenas a conta somente leitura `convidado@yaboa.app`, sem apagar usuários existentes.
4. Use `npm run redefinir:usuarios` somente quando quiser apagar deliberadamente todas as contas de Auth e recriar o convidado.
5. Para usar a mesma conta no navegador, mantenha os valores `VITE_YABOA_GUEST_EMAIL` e `VITE_YABOA_GUEST_PASSWORD` iguais aos valores `YABOA_GUEST_EMAIL` e `YABOA_GUEST_PASSWORD` usados pelo script administrativo.
